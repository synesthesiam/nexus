...
namespace Nexus.Shared
{
    ...
    public static int LoadSingle(string basePath, string name)
    {
      ...
      var potWidth = (int)Math.Pow(2, Math.Ceiling(Math.Log(bitmap.Width, 2)));
      var potHeight = (int)Math.Pow(2, Math.Ceiling(Math.Log(bitmap.Height, 2)));
      var potBitmap = new Bitmap(potWidth, potHeight);

      using (var g = Graphics.FromImage(potBitmap)) {
        g.FillRectangle(Brushes.Transparent, 0, 0, potWidth, potHeight);
        g.DrawImage(bitmap, 0, 0);
      }
        	
      // Load raw bitmap data into OpenGL
			var bitmapData = potBitmap.LockBits(new Rectangle(0, 0,
			...
			potBitmap.UnlockBits(bitmapData);
